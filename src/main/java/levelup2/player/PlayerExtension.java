...
public class PlayerExtension implements IPlayerClass
{
    ...
    public CompoundNBT saveNBTData(CompoundNBT tag)
    {
        ...
        Map<String, CompoundNBT> types = Maps.newHashMap();
        ...
        for (ResourceLocation skillName : skillMap.keySet())
        {
            ...
            {
                types.put(type, new CompoundNBT());
            }
    ...
    public void loadNBTData(CompoundNBT tag) 
    {
        if (!tag.contains("Version"))
        {
            for (ResourceLocation skill : skillMap.keySet()) 
            {
                ...
                CompoundNBT skillTag = tag.getCompound(skillName);
    ...
    @Override
    public boolean addLevelFromExperience(PlayerEntity player)
    {
